...
public enum SystemUtility {
    ...
    /// - Parameters:
    ///   - args: An array starting with the command and any following arguments as separate `Strings` in the array.
    ///   - launchPath: path the base executable.  Defaults value is `"/usr/bin/env"` and can be ignored in most cases.
    ...
    /// - Parameters:
    ///   - args: An array starting with the command and any following arguments as separate Strings in the array.
    ///   - launchPath: path the base executable. Default value is `"/usr/bin/env"` and can be ignored in most cases.
    /// - Returns: tuple with the following structure: `(returnCode: Int32, stdOut: [String], stdError: [String])`
    ///
    /// Usage example:
    /// ```
