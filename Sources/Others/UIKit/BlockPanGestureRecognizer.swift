...
public class BlockPanGestureRecognizer: UIPanGestureRecognizer {
    ...
    #if os(iOS)
    public  convenience init (minimumNumberOfTouches: Int, panAction: ((UIPanGestureRecognizer) -> Void)?) {
        self.init(panAction: panAction)
        self.minimumNumberOfTouches = minimumNumberOfTouches
    }
    #endif

    public  convenience init (panAction: ((UIPanGestureRecognizer) -> Void)?) {
        self.init()
